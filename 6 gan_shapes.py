...
generator = tf.keras.Sequential([
    ...
    tf.keras.layers.Reshape((7, 7, 256)), tf.keras.layers.Conv2DTranspose(128, (5, 5), strides=(1, 1),padding='same', use_bias=False),
...
def train_step(images):
    ...
    with tf.GradientTape() as gen_tape:
        ...
        gen_preds = discriminator(generated_images, training=False) # Calculate generator's loss
...
def train_step(images):
    # Generate noise
    noise = tf.random.normal([BATCH_SIZE, noise_dim])
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape: # Generate fake images
        generated_images = generator(noise, training=True)
...
def generate_and_save_images(model, epoch, test_input): # Generate images from the model
